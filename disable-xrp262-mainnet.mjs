...
async function main() {
  ...
  const tx = new TransactionBuilder(await server.getAccount(deployer.publicKey()), {
    ...
    contract: LEXORA, function: "disable_token", args: [assetId()],
  })).setTimeout(300).build();

  console.log("XRP262 MAINNET REGISTRY DISABLE");
  ...
  if (result.status === "FAILED") {
    ...
    throw new Error("XRP262 registry disable failed.");
  }
  console.log("XRP262 registry disabled successfully.");
}
