...
async function main() {
  ...
  const tx = new TransactionBuilder(await server.getAccount(deployer.publicKey()), {
    ...
    contract: LEXORA, function: "register_token", args: [{
      code: nativeToScVal("XRP262", { type: "string" }),
      issuer: Address.fromString(XRP262_ISSUER).toScVal(),
    }],
  })).setTimeout(300).build();
  ...
  if (result.status === "FAILED") {
    ...
    throw new Error("XRP262 registry registration failed.");
  }
  console.log("XRP262 registry registration succeeded.");
}
