import { generateTypeImports, formatImports, sanitizeIdentifier, toCamelCase, propertyKey, parseTypeFromTypeDef, formatJSDocComment, escapeStringLiteral } from './utils.js';
import '../xdr/index.js';
import { ScSpecEventParamLocationV0 } from '../xdr/generated/sc-spec-event-param-location-v0.js';
...
${eventMethods}
    ...
    if (events.length > 0) {
      ...
      events.forEach((event) => {
        const topicParams = event.params.filter(
          (param) => param.location.value === ScSpecEventParamLocationV0.scSpecEventParamLocationTopicList.value
        );
  ...
  parseEventMethodName() {
    const reserved = new Set(
      this.spec.funcs().filter((func) => func.name.toString() !== "__constructor").map((func) => sanitizeIdentifier(func.name.toString()))
    );
  ...
  generateDeployMethod(constructorFunc) {
    const inputs = constructorFunc?.inputs.map(ClientGenerator.methodInput) ?? [];
   ...
   * that name.
   */
  static methodInput(input) {
    return {
      key: propertyKey(input.name.toString()),
      type: parseTypeFromTypeDef(input.type, true)
    };
  }
  static inputsType(inputs) {
    return `{ ${inputs.map((i) => `${i.key}: ${i.type}`).join("; ")} }`;
  }
  static DEPLOY_OPTIONS_PARAM = `options: MethodOptions & Omit<ContractClientOptions, 'contractId'> & { salt?: Uint8Array; address?: string; } & ({ wasmHash: Uint8Array | string; format?: "hex" | "base64"; externalRef?: never; } | { externalRef: ExternalExecutableRef; wasmHash?: never; format?: never; })`;
  /**
   ...
   */
  static formatParameters(inputs, optionsParam = "options?: MethodOptions") {
    const params = [];
    if (inputs.length > 0) {
