...
class TypeGenerator {
  ...
  generateEntry(entry) {
    switch (entry.type) {
      case "scSpecEntryUdtStructV0":
        if (utils.isTupleStruct(entry.value)) {
  ...
  generateStruct(struct) {
    ...
    const fields = struct.fields.map((field) => {
      const fieldName = utils.propertyKey(field.name.toString());
...
${fields}
  ...
  generateUnion(union) {
    ...
    const doc = utils.formatJSDocComment(union.doc.toString() || `Union: ${name}`, 0);
  ...
  resolveEventInterfaceNames() {
    ...
    for (const entry of this.spec.entries) {
      switch (entry.type) {
        ...
        case "scSpecEntryUdtEnumV0":
        case "scSpecEntryUdtErrorEnumV0":
          reserved.add(utils.sanitizeIdentifier(entry.value.name.toString()));
          break;
      }
    }
...
Note: renamed from "${preferred}" to avoid a collision with another generated name.` : "";
    ...
    const dataIsMapFormat = event.dataFormat.value === scSpecEventDataFormat.ScSpecEventDataFormat.scSpecEventDataFormatMap.value;
    const dataFields = event.params.map((param) => {
      const fieldName = utils.propertyKey(param.name.toString());
      const fieldType = utils.parseTypeFromTypeDef(param.type);
      const fieldDoc = utils.formatJSDocComment(param.doc.toString(), 4);
      const optional = dataIsMapFormat && param.location.value === scSpecEventParamLocationV0.ScSpecEventParamLocationV0.scSpecEventParamLocationData.value;
