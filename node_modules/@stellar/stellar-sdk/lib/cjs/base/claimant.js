...
var claimPredicate = require('../xdr/generated/claim-predicate.js');
var claimantV0 = require('../xdr/generated/claimant-v0.js');
var claimant = require('../xdr/generated/claimant.js');
...
class Claimant {
  ...
  constructor(destination, predicate) {
    ...
    if (!predicate) {
      this._predicate = claimPredicate.ClaimPredicate.claimPredicateUnconditional();
    } else if (predicate instanceof claimPredicate.ClaimPredicate) {
  ...
  static predicateUnconditional() {
    return claimPredicate.ClaimPredicate.claimPredicateUnconditional();
  }
  /**
  ...
  static predicateAnd(left, right) {
    if (!(left instanceof claimPredicate.ClaimPredicate)) {
      throw new Error("left Predicate should be an xdr.ClaimPredicate");
    }
    if (!(right instanceof claimPredicate.ClaimPredicate)) {
      throw new Error("right Predicate should be an xdr.ClaimPredicate");
    }
    return claimPredicate.ClaimPredicate.claimPredicateAnd([left, right]);
  }
  /**
  ...
  static predicateOr(left, right) {
    ...
    return claimPredicate.ClaimPredicate.claimPredicateOr([left, right]);
  }
  /**
  ...
  static predicateNot(predicate) {
    if (!(predicate instanceof claimPredicate.ClaimPredicate)) {
      throw new Error("Predicate should be an xdr.ClaimPredicate");
    }
    return claimPredicate.ClaimPredicate.claimPredicateNot(predicate);
  }
  /**
  ...
  static predicateBeforeAbsoluteTime(absBefore) {
    return claimPredicate.ClaimPredicate.claimPredicateBeforeAbsoluteTime(
      index.Int64.fromString(absBefore)
    );
  }
  /**
  ...
  static predicateBeforeRelativeTime(seconds) {
    return claimPredicate.ClaimPredicate.claimPredicateBeforeRelativeTime(
      index.Int64.fromString(seconds)
    );
  }
  /**
  ...
  static fromXdr(claimantXdr) {
    ...
    switch (claimantXdr.type) {
      case "claimantTypeV0":
        ...
        return new this(
          strkey.StrKey.encodeEd25519PublicKey(value.destination.value.toBytes()),
          value.predicate
        );
      default:
   ...
   */
  toXdrObject() {
    const claimant$1 = new claimantV0.ClaimantV0({
      destination: keypair.Keypair.fromPublicKey(this._destination).xdrAccountId(),
      predicate: this._predicate
    });
    return claimant.Claimant.claimantTypeV0(claimant$1);
  }
  /**
