...
function findCreatedAccountSequenceInTransactionMeta(meta) {
  ...
  const created = operations.flatMap((op) => op.changes).find(
...
class RpcServer {
  ...
  async getAccountEntry(address) {
    ...
    try {
      const resp = await this.getLedgerEntry(ledgerKey$1);
  ...
  async getTrustline(account, asset) {
    const trustlineLedgerKey = ledgerKey.LedgerKey.trustline(
  ...
  async getClaimableBalance(id) {
    ...
    if (strkey.StrKey.isValidClaimableBalance(id)) {
      const buffer = strkey.StrKey.decodeClaimableBalance(id);
      const v = uint8arrayExtras.concatUint8Arrays([new Uint8Array(3), buffer.subarray(0, 1)]);
      balanceId = claimableBalanceId.ClaimableBalanceId.fromXdr(
        uint8arrayExtras.concatUint8Arrays([v, buffer.subarray(1)])
      );
    } else if (id.match(/[a-f0-9]{72}/i)) {
      balanceId = claimableBalanceId.ClaimableBalanceId.fromXdr(id, "hex");
    } else if (id.match(/[a-f0-9]{64}/i)) {
      balanceId = claimableBalanceId.ClaimableBalanceId.fromXdr(id.padStart(72, "0"), "hex");
    } else {
      throw new TypeError(`expected 72-char hex ID or strkey, not ${id}`);
    }
    const trustlineLedgerKey = ledgerKey.LedgerKey.claimableBalance(
      new ledgerKeyClaimableBalance.LedgerKeyClaimableBalance({ balanceId })
    );
    try {
      const entry = await this.getLedgerEntry(trustlineLedgerKey);
      if (entry.val.type !== "claimableBalance") {
        throw new Error(
          "unexpected ledger entry type for claimable balance: " + entry.val.type
        );
      }
      return entry.val.value;
    } catch {
  ...
  async getContractData(contract$1, key, durability = "persistent" /* Persistent */) {
    ...
    switch (durability) {
      case "temporary" /* Temporary */:
        xdrDurability = contractDataDurability.ContractDataDurability.temporary;
        break;
      case "persistent" /* Persistent */:
        xdrDurability = contractDataDurability.ContractDataDurability.persistent;
        break;
      default:
        throw new TypeError(`invalid durability: ${durability}`);
    }
    const contractKey = ledgerKey.LedgerKey.contractData(
  ...
  async getExternalRefWasmHash(ref) {
    ...
    if (owner.type !== "scAddressTypeContract") {
      return Promise.reject({
        code: 400,
        message: `External executable owner ${address.Address.fromScAddress(owner)} is not a contract, so it cannot hold the tag entry that names the Wasm`
      });
    }
    const entry = await this.getContractData(
      address.Address.fromScAddress(owner),
      scVal.ScVal.scvExecutableTag(ref.tag),
    ...
    if (scv.type !== "scvBytes" || scv.bytes.value.length !== 32) {
      return Promise.reject({
        code: 404,
        message: `External executable tag entry on ${address.Address.fromScAddress(owner)} does not hold a 32-byte Wasm hash`
      });
    }
  ...
  async getContractWasmByHash(wasmHash, format = void 0) {
    ...
    if (typeof wasmHash === "string") {
      if (format === "base64") {
        wasmHashBytes = base64.base64ToUint8Array(wasmHash);
      } else if (format === "hex") {
        wasmHashBytes = uint8arrayExtras.hexToUint8Array(wasmHash);
      } else {
        wasmHashBytes = uint8arrayExtras.stringToUint8Array(wasmHash);
      }
    } else {
      wasmHashBytes = wasmHash;
    }
    const ledgerKeyWasmHash = ledgerKey.LedgerKey.contractCode(
      new ledgerKeyContractCode.LedgerKeyContractCode({
        hash: new hash.Hash(wasmHashBytes)
      })
    );
  ...
  async queryContract(contractId, method, args = {}, networkPassphrase) {
    ...
    const { Client } = await Promise.resolve().then(function () { return require('../contract/client.js'); });
    ...
    const { sanitizeIdentifier } = await Promise.resolve().then(function () { return require('../bindings/utils.js'); });
   ...
   * ```
   */
  async simulateTransaction(tx, addlResources, authMode, useUpgradedAuth = true) {
    return this._simulateTransaction(
      tx,
      addlResources,
      authMode,
      useUpgradedAuth
    ).then(parsers.parseRawSimulation);
  }
  async _simulateTransaction(transaction, addlResources, authMode, useUpgradedAuth = true) {
    return jsonrpc.postObject(
      this.httpClient,
      this.serverURL.toString(),
      "simulateTransaction",
   ...
   * ```
   */
  async prepareTransaction(tx, useUpgradedAuth = true) {
  ...
  async requestAirdrop(address, friendbotUrl) {
    ...
    try {
      ...
      if (!response.data.result_meta_xdr) {
        ...
        meta = transactionMeta.TransactionMeta.fromXdr(response.data.result_meta_xdr, "base64");
      }
  ...
  async getSACBalance(address$1, sac, networkPassphrase) {
    ...
    const ledgerKey$1 = ledgerKey.LedgerKey.contractData(
      new ledgerKeyContractData.LedgerKeyContractData({
    ...
    const response = await this.getLedgerEntries(ledgerKey$1);
    ...
    const entry = scval.scValToNative(val.value.val);
    return {
