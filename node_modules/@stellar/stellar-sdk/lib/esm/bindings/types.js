import { isTupleStruct, generateTypeImports, formatImports, sanitizeIdentifier, formatJSDocComment, propertyKey, parseTypeFromTypeDef, escapeStringLiteral, toPascalCase } from './utils.js';
import '../xdr/index.js';
import { ScSpecEventDataFormat } from '../xdr/generated/sc-spec-event-data-format.js';
...
class TypeGenerator {
  ...
  generate() {
    ...
    const types = this.spec.entries.map(
      (entry) => entry.type === "scSpecEntryEventV0" ? this.generateEvent(entry.value, eventIndex++) : this.generateEntry(entry)
    ).filter((t) => t).join("\n\n");
...
${fields}
  ...
  generateUnion(union) {
    ...
    const cases = union.cases.map(
      (unionCase) => this.generateUnionCase(unionCase)
    );
...
${members}
  ...
  generateErrorEnum(errorEnum) {
    ...
    const cases = errorEnum.cases.map(
      (enumCase) => this.generateEnumCase(enumCase)
    );
...
${members}
  ...
  generateUnionCase(unionCase) {
    switch (unionCase.type) {
      ...
      default:
        throw new Error(
          `Unknown union case kind: ${unionCase.type}`
        );
    }
  }
  /**
...
Note: renamed from "${preferred}" to avoid a collision with another generated name.` : "";
    ...
    const dataIsMapFormat = event.dataFormat.value === ScSpecEventDataFormat.scSpecEventDataFormatMap.value;
    const dataFields = event.params.map((param) => {
      ...
      const optional = dataIsMapFormat && param.location.value === ScSpecEventParamLocationV0.scSpecEventParamLocationData.value;
...
${dataFields}
  ...
  generateContractEventUnion() {
    ...
    const names = eventEntries.map(
      (entry, eventIndex) => this.eventInterfaceName(entry.value, eventIndex)
    );
